...
class OandaEnv:
	...
	def downloadHistory(self,GI):
		...
		for InstI in range(0,len(self.Instruments)):
			...
			for i in range(0,days):
				...
				print(StartDate)
				print(EndDate)
				downloadHist[InstI][i] = self.InstrumentDownload(self.Granularity[GI],self.Instruments[InstI],StartDate,EndDate)
	...
	def CloseLong(self,InstrumentID):
		...
		profit = (self.CurrentPrice[InstrumentID]-self.LongPositions[InstrumentID,self.LastLongID[InstrumentID]]) * self.lot[InstrumentID]
	...
	def CloseShort(self,InstrumentID):
		...
		profit = (self.CurrentPrice[InstrumentID]-self.ShortPositions[InstrumentID,self.LastShortID[InstrumentID]]) * self.lot[InstrumentID]
		self.equity = self.equity + profit - self.fee
	...
	def getHistory(self):
		...
		for GI in range(0,len(self.Granularity)):
			...
			for fj in range(0,j):
				...
				priceDELTA[:,fj] = self.PriceScallarsD[GI][fj].transform(priceDELTA[:,fj].reshape(-1, 1)).reshape((i,))
			...
			for fj in range(0,j):
				...
				# print(self.MinMaxScallarsD[GI][fj].data_max_)
				volumeData[:,fj] = self.VolumeScallarsD[GI][fj].transform(volumeData[:,fj].reshape(-1, 1)).reshape((i,))
	...
	def CalcIndicatorsFull(self,priceDELTA,volumeData,GI):
		...
		for li in range(self.MeanMax+self.PastDataForStep, i):
			if(max(priceDELTA[li-self.MeanMax:li,6])-min(priceDELTA[li-self.MeanMax:li,9]) == 0 ):
				...
				osc1 =(priceDELTA[li,0]-min(priceDELTA[li-self.MeanMax:li,9])) / (max(priceDELTA[li-self.MeanMax:li,6])-min(priceDELTA[li-self.MeanMax:li,9]))
			if(max(priceDELTA[li-self.MeanMax:li,7])-min(priceDELTA[li-self.MeanMax:li,10]) == 0 ):
				osc2 = 0
			else:
				osc2 =(priceDELTA[li,1]-min(priceDELTA[li-self.MeanMax:li,10])) / (max(priceDELTA[li-self.MeanMax:li,7])-min(priceDELTA[li-self.MeanMax:li,10]))
			if(max(priceDELTA[li-self.MeanMax:li,8])-min(priceDELTA[li-self.MeanMax:li,11]) == 0 ):
				osc3 = 0
			else:
				osc3 =(priceDELTA[li,3]-min(priceDELTA[li-self.MeanMax:li,11])) / (max(priceDELTA[li-self.MeanMax:li,8])-min(priceDELTA[li-self.MeanMax:li,11]))
