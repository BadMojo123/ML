...
modelName = 'klony/Mieszko1527519504.3214564.h5'
ClassifierName = 'klony/MieszkoGraph1527519504.3214564.h5'
...
def CheckForNewData(TickData,order):
	...
	if msgArray[0]=='DATA': 
		...
		elif(prediction[2]==1 and order!="SELL"):
			order="SELL"
			print('[RYCHEZA] Sending CLOSE BUY order')
			remote_send(eurusd_close_orders)
			print('[RYCHEZA] Sending SELL order')
			remote_send(eurusd_sell_order)
		elif(prediction[1]==1 and order=="SELL"):
			remote_send(eurusd_close_orders)
			order="CLOSE"
		elif(prediction[3]==1 and order=="BUY"):
			remote_send(eurusd_close_orders)
			order="CLOSE"
...
def PrepareForPrediction(DATA,Hist=80,samples=1):
	...
	if(samples==1):
		...
		i,j = DATA.shape
		DATA = np.concatenate((DATA[0:i-2],DATA[1:i-1,7].reshape((i-2,1)),DATA[2:i,7].reshape((i-2,1))),axis=1) 
