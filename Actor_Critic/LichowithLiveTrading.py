...
from keras.layers import Input, Dense, Dropout, Conv2D, Flatten,MaxPooling2D,LSTM,Lambda, Permute
...
class Licho:
	...
	def build_actor_continuous(self):
		...
		t0State =Lambda(lambda tfAt0State: tfAt0State[:,:,-1,:], output_shape=(self.TimeFramesLen,self.NormedFeaturesLen,))(tfAt0State)
	...
	def get_batch(self):
		...
		for li in range(self.BATCH_SIZE):
			...
			action_matrix, predicted_action = self.Act(Time.reshape((1,123)),
					predicted_action,
					observation.reshape((1,self.TimeFramesLen,self.PastDataForStep,self.NormedFeaturesLen)),
		...
		return batchObservation, batchAction, batchPredicted_action, batchreward, batchTime.reshape((self.BATCH_SIZE,123)),bachCurrentPrice
	...
	def train(self):
		...
		while self.episode < self.EPISODES:
			...
			for e in range(self.EPOCHS):
				...
				actor_loss.append(self.actor.train_on_batch([obs, advantage, old_prediction,Time,CurrentPrice], [action]))
